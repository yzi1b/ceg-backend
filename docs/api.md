...
3. [API 端点](#3-api-端点)
   ...
   - 3.13 [学生列表](#313-get-coursestudents)
   - 3.14 [删除课程](#314-delete-courseobject)
   - 3.15 [修改课程](#315-post-courseobject)
   - 3.16 [创建考试](#316-post-examcreate)
   - 3.17 [考试列表](#317-get-examlist)
   - 3.18 [考试详情](#318-get-examobject)
   - 3.19 [修改考试](#319-post-examobject)
   - 3.20 [删除考试](#320-delete-examobject)
   - 3.21 [切换考试阶段](#321-post-examstage)
   - 3.22 [创建试卷](#322-post-papercreate)
...
### 3.13 GET /course/students

获取课程的班级学生列表。

> **需要认证：** 教师（`teacher`）及以上角色，且只能查看自己课程的学生列表（管理员可查看任意课程）。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `objects` | array | 学生信息数组（格式同 [GET /user/object](#35-get-userobject)） |
| `count` | integer | 学生总数 |
...
| 400 | - | `{}` | 参数缺失或格式错误 |
| 401 | -1 | `token is not provided, invalid or expired` | 未认证 |
| 403 | -1 | `permission required` | 权限不足（非教师/管理员，或非本课程教师） |
| 404 | - | `{}` | 课程不存在 |
| 500 | - | `{}` | 服务器内部错误 |
...
```bash
curl -X GET "http://localhost:8088/course/students?id=92345678" \
  -H "Authorization: Bearer <token>"
```

---

### 3.14 DELETE /course/object
